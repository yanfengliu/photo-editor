...
export function CurveEditor({ points, onChange, color = "white" }: Props) {
  ...
  const fromEvent = useCallback((e: React.MouseEvent): CurvePoint => {
    const rect = svgRef.current!.getBoundingClientRect();
    const svgX = ((e.clientX - rect.left) / rect.width) * SIZE;
    const svgY = ((e.clientY - rect.top) / rect.height) * SIZE;
    return { x: Math.max(0, Math.min(1, (svgX - PAD) / (SIZE - 2 * PAD))), y: Math.max(0, Math.min(1, 1 - (svgY - PAD) / (SIZE - 2 * PAD))) };
  }, []);
  ...
  }, [dragging, points, onChange, fromEvent]);
  ...
  return (
    <svg ref={svgRef} viewBox={`0 0 ${SIZE} ${SIZE}`} className={styles.svg} onMouseMove={handleMouseMove} onMouseUp={() => setDragging(null)} onMouseLeave={() => setDragging(null)} onDoubleClick={handleDoubleClick}>
